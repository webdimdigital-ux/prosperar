...
export function AdminExamListPage() {
  const { data, paginatorInfo, loading, search, setSearch, page, setPage, perPage, setPerPage,
    sortColumn, sortOrder, handleSort, filter, setFilter, refetch } = useExamList()
  ...
  const columns = [
    ...
    { key: 'hospital', label: 'Unidade', render: (r: ExamRow) => r.hospital?.name },
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={!!editExam} onOpenChange={open => !open && setEditExam(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Editar exame #{editExam?.id}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleEditSave} className="space-y-3">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setEditExam(null)}>Cancelar</Button>
